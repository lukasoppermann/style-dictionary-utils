...
describe('index.ts', () => {
  ...
  it('all formats are attached', () => {
    expect(StyleDictionary.hooks.formats['css/advanced']).toBeDefined()
  ...
  it('can be extended with a transform', () => {
    StyleDictionary.registerTransform({
      name: "test/valueTransformer",
      type: `value`,
      transitive: true,
      transform: () => 'test'
    })
    expect(StyleDictionary.hooks.transforms['test/valueTransformer']).toEqual({
      "filter": undefined,
  ...
  it('can be extended with a parser', () => {
    StyleDictionary.registerParser({
      name: 'testParser',
      pattern: /\.json$/,
      parser: () => ({ test: { value: "test" } })
    })
    expect(StyleDictionary.hooks.parsers["testParser"]).toEqual({
      "parser": expect.any(Function),
      "pattern": /\.json$/,
    })
  })
