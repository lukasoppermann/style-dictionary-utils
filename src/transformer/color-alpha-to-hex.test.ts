...
import { colorAlphaToHex } from './color-alpha-to-hex.js';
...
describe('Transformer: colorAlphaToHex', () => {
  ...
  it('transforms `color` tokens with hex value', () => {
    expect([
      ...
      { value: '#343434' },
      { value: '#34343466' }
    ].map(item => colorAlphaToHex.transform(item as TransformedToken, {}, {}))).toStrictEqual([
      "#334433",
      "#343434",
      "#34343466"
    ]);
  ...
  it('transforms `color` tokens with alpha value', () => {
    ...
    ].map(item => colorAlphaToHex.transform(item, {}, {}))).toStrictEqual([
      "#34343466",
      "#34343433"
    ]);
  })
