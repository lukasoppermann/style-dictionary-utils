...
import { colorAlphaToRgba } from './color-alpha-to-rgba.js';
...
describe('Transformer: colorToRgba', () => {
  ...
  it('transforms `color` tokens with hex value', () => {
    expect([
      ...
      { value: '#343434' },
      { value: '#34343466' }
    ].map(item => colorAlphaToRgba.transform(item as TransformedToken, {}, {}))).toStrictEqual([
      "rgba(51, 68, 51, 1)",
      "rgba(52, 52, 52, 1)",
      "rgba(52, 52, 52, 0.4)"
    ]);
  ...
  it('transforms `color` tokens with alpha', () => {
    ...
    ].map(item => colorAlphaToRgba.transform(item, {}, {}))).toStrictEqual([
      "rgba(52, 52, 52, 0.4)",
      "rgba(52, 52, 52, 0.2)",
      "rgba(52, 52, 52, 0)",
    ]);
