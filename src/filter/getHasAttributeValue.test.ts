...
describe('Filter: getHasAttributeValue', () => {
  ...
  it('filters type tokens', () => {
    ...
    expect(items.filter(getHasAttributeValue(['deprecated', '$deprecated'], [true, false]))).toStrictEqual([
      items[0],
      items[1],
    ])
    expect(items.filter(getHasAttributeValue(['number', 'value'], [{isValid: true}, '2rem', 1]))).toEqual([
      items[1],
      items[2],
      items[3],
    ])
