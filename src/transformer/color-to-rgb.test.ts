...
describe('transform: colorToHex', () => {
  it('transforms `color` tokens with hex value', () => {
    expect(
      [{value: '#343'}, {$value: '#343434'}, {value: '#34343466'}].map(item =>
        colorToRgba.transform(item as TransformedToken, {}, {}),
      ),
  ...
  it('transforms `color` tokens with rgb value', () => {
    ...
    ).toStrictEqual(['rgba(100, 200, 255, 1)', 'rgba(100, 200, 255, 0.4)'])
  })
  ...
  it('transforms `color` tokens and ignores alpha value', () => {
    expect(
      ...
      ].map(item => colorToRgba.transform(item)),
    ).toStrictEqual(['rgba(52, 52, 52, 1)', 'rgba(52, 52, 52, 0.4)'])
  })

  it('transforms `named colors` and `transparent` to rgb value', () => {
    const input = [{value: 'purple'}, {$value: 'transparent'}]
    const expectedOutput = ['rgba(128, 0, 128, 1)', 'rgba(0, 0, 0, 0)']
    expect(input.map(item => colorToRgba.transform(item as TransformedToken, {}, {}))).toStrictEqual(expectedOutput)
  })
})
