...
describe('transform: colorToHex', () => {
  ...
  it('transforms `color` tokens with hex value', () => {
    expect([
      ...
      { value: '#343434' },
      { value: '#34343466' }
    ].map(item => colorToHex.transform(item as TransformedToken, {}, {}))).toStrictEqual([
      "#334433",
      "#343434",
      "#34343466"
    ]);
  ...
  it('transforms `color` tokens and ignores alpha value', () => {
    ...
    ].map(item => colorToHex.transform(item))).toStrictEqual([
      "#343434",
      "#34343466"
    ]);
  })
  ...
  it('transforms `named colors` and `transparent` to hex value', () => {
    ...
    expect(input.map(item => colorToHex.transform(item as TransformedToken, {}, {}))).toStrictEqual(expectedOutput)
  })
})