...
describe('Transformer: shadowCss', () => {
  ...
  const filterFn = (item: TransformedToken) => (shadowCss.filter as Filter['filter'])(item, {})
  ...
  it('transforms `shadow` tokens', () => {
    expect(items.filter(filterFn).map(item => shadowCss.transform(item, {}, {}))).toStrictEqual([
  ...
  it('should transform an array of shadow values', () => {
    ...
    expect(shadows.filter(filterFn).map(item => shadowCss.transform(item, {}, {}))).toStrictEqual([
